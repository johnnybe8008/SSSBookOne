...
// Calls /api/oauth/mobile endpoint which returns JSON with app_session_id and staff
export async function exchangeOAuthCode(
  code: string,
  state: string,
): Promise<{ sessionToken: string; staff: any }> {
  console.log("[API] exchangeOAuthCode called");
  const params = new URLSearchParams({ code, state });
  const endpoint = `/api/oauth/mobile?${params.toString()}`;
  console.log("[API] Calling OAuth mobile endpoint:", endpoint);
  const result = await apiCall<{ app_session_id: string; staff: any }>(endpoint);
...
// Get current authenticated staff (web uses cookie-based auth)
export async function getMe(): Promise<{
  ...
  try {
    const result = await apiCall<{ staff: any }>("/api/auth/me");
